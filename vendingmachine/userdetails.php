...
if(isset($_GET['d_id']) and isset($_GET['amount'])  )// and isset($_POST['newDeviceactive'])
{
  include 'conn.php';
  $id = $_GET['d_id'];
  $amount = $_GET['amount'];
...
//   echo($sql);
 ...
 $sql = "INSERT INTO `history` (`rfid`, `type`, `amount`, `note`) VALUES ('$id', 'CRADIT', '$amount', 'Recharge');";
 //echo($sql);
 if ($conn->query($sql) === TRUE) {
   //  echo "Recharge successfully";
	     $status = "1";
			  ...
			  if(isset($_GET['d_id']))
			  {
                ...
                <div class="card-body">
                  <div class="form-group">
                    ...
                    <label for="exampleInputEmail1">RFID : <?php  echo($row['rfid'])?></label><br>
                    <label for="exampleInputEmail1">Available Balance : <?php  echo($row['balance'])?></label><br>
                    <form mathod="get" action="userdetails">
					<input type="number" name = "amount" class="form-control form-control-sm" placeholder="" aria-controls="example1">
					<br>
					<input name="d_id" value="<?php  echo($row['rfid'])?>" type="hidden">
					<button type="submit" class="btn btn-primary row-address">Recharge</button>
